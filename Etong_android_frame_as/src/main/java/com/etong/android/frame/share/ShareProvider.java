...
public class ShareProvider {
	...
	public static ShareProvider getInstance() {
		if (null == instance) {
			...
			api = WXAPIFactory.createWXAPI(BaseApplication.getApplication()
	 ...
	 * @params
	 * @param info
	 *            分享内容</br>文本（{@link ShareInfo.text} 必选）
	 * @param type
	...
	public boolean SendTextToWX(ShareInfo info, boolean type, String tag) {
		...
		if (!api.sendReq(req)) {
			...
			L.e("ShareProvider:" + tag, "IWXAPI sendReq fail ");
			return false;
		} else {
			return true;
		}
	 ...
	 * @params
	 * @param info
	 *            分享内容</br>图片（{@link ShareInfo.imageBitmap}/
	 *            {@link ShareInfo.imagePath} 必选），图片地址（
	 *            {@link ShareInfo.imageUrl} 可选)
	 * @param type
	 *            true:发送到朋友圈 ;false:发送到聊天界面
	 * @param tag
	 *            请求标识
	 ...
	 * @params
	 * @param info
	 *            分享内容</br>标题（{@link ShareInfo.title} 必选），文本（
	 *            {@link ShareInfo.text} 必选），网址（{@link ShareInfo.Url} 必选），图片（
	 *            {@link ShareInfo.imageBitmap}/{@link ShareInfo.imagePath} 可选）
	 * @param type
	 *            true:发送到朋友圈 ;false:发送到聊天界面
	 * @param tag
	 *            请求标识
