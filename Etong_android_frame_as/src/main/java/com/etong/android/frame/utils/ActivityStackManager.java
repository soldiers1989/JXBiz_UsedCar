    ...
    public void finishActivity(Class<?> cls) {
        ...
        for (BaseSubscriberActivity activity : activityStack) {
            if (activity.getClass().equals(cls)) {
                selectActivity = activity;//在循环中remove掉activity会出现ConcurrentModificationException，所以先赋值，循环完再remove
                break;
//	                finishActivity((Activity) activity);//在循环中remove掉activity会出现ConcurrentModificationException
            }
        }
        finishActivity((Activity) selectActivity);
    }
