 ...
 * Menu中在售车型的适配器
 */
public class FC_GrandInsalesAdapter extends ArrayAdapter<FC_InsalesListItem> implements PinnedSectionListView.PinnedSectionListAdapter {

    private Context mContext;

    private List<FC_InsalesTotalItem> mListDatas;            // 传递过来的数据

    public FC_GrandInsalesAdapter(Context context) {
        super(context, android.R.layout.simple_list_item_1);
        this.mContext = context;
        mListDatas = new ArrayList<FC_InsalesTotalItem>();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (item.type == FC_InsalesListItem.SECTION) {
            ...
            if(null==item.getMinguide() || null==item.getMaxguide()){
                rBText.setText("未记录");
            }else{
