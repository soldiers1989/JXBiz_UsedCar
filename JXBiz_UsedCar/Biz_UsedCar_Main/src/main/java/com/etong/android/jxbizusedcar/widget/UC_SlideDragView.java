...
/**
 * @desc (这里用一句话描述这个类的作用)
 * @createtime 2016/10/31 0031--19:23
 * @Created by wukefan.
 */
public class UC_SlideDragView extends RelativeLayout implements View.OnClickListener{
        ...
        public UC_SlideDragView(Context context) {
            super(context);
        }

        public UC_SlideDragView(Context context, AttributeSet attrs) {
            super(context, attrs);
            ...
            mDrager = ViewDragHelper.create(this, 5f, new ViewDragHelper.Callback() {
                ...
                public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
                    ...
                    if(mDragStateListener != null){
                        if(left == 0){
                            mDragStateListener.onClosed(UC_SlideDragView.this);
                        }else if(Math.abs(left) == bgView.getMeasuredWidth()){
                            mDragStateListener.onOpened(UC_SlideDragView.this);
                        }
                    }
                }
            });
        ...
        public void onClick(View v) {
            if(mDragStateListener != null) {
                if(v == fgView){
                    ...
                    mDragStateListener.onForegroundViewClick(UC_SlideDragView.this , v);
                }else {
                    mDragStateListener.onBackgroundViewClick(UC_SlideDragView.this , v);
                }
            }
        }

        public interface DragStateListener{
            void onOpened(UC_SlideDragView dragView);
            void onClosed(UC_SlideDragView dragView);
            void onForegroundViewClick(UC_SlideDragView dragView, View v);
            void onBackgroundViewClick(UC_SlideDragView dragView, View v);
        }
