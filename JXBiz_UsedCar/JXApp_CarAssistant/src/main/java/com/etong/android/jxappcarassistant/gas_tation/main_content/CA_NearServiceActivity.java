...
//                Toast.makeText(MainActivity.this, "请去应用管理中设置该app的定位权限为允许！", Toast.LENGTH_LONG).show();

//                Toast.makeText(MainActivity.this, "请选择允许！", Toast.LENGTH_LONG).show();
                initPermission();
//                appops();
            }
...
//    }
     ...
     * @desc 获取今日油价信息
     */
    @Subscriber(tag = UC_HttpTag.OIL_PRICE)
...
//        JSONObject data = JSON.parseObject(jsonData);//json string 转json object
     ...
     * @desc 获取附近加油站信息
     */
    @Subscriber(tag = UC_HttpTag.OIL_LOCAL)
                ...
                .requestPermissionsIfNecessaryForResult(this,
                        ...
                        new PermissionsResultAction() {
                            ...
                            public void onDenied(String permission) {
                                toastMsg("您禁止了定位权限，我们无法定位，请去应用管理中打开本应用的定位权限哦~");
                                return;
                            }
                        });
    }
...
//        option.setOpenGps(true);//可选，默认false,设置是否使用gps
    ...
    public class MyLocationListenner implements BDLocationListener {
        ...
        public void onReceiveLocation(BDLocation location) {
            ...
            Log.d("~~~~~~~~~", location.getLocType() + "");//错误码信息


            if (location.getLocType() == 62) {
                if (isFirstLoc) {
                    UC_FrameEtongApplication.getApplication().setIsLocation(false);
                    EtongToast.makeText(CA_NearServiceActivity.this,
                            "定位失败,请检查运营商网络或者wifi网络是否正常开启或检查应用管理中本app是否开启相关权限,尝试重新请求定位",
                            Toast.LENGTH_LONG);
                    isFirstLoc = false;
                }
            }
...
//                        .radius(radius).pageNum(loadIndex);
            ...
            if (location == null || mMapView == null) {
                EtongToast.makeText(CA_NearServiceActivity.this, "定位失败，请检查手机网络或设置！", EtongToast.LENGTH_SHORT).show();
                return;
            }
        }
...
//            CToast.toastMessage("未找到结果", 1);
            EtongToast.makeText(CA_NearServiceActivity.this, "未找到结果", EtongToast.LENGTH_SHORT).show();
            return;
        }
...
//            CToast.toastMessage(strInfo, 1);
            EtongToast.makeText(CA_NearServiceActivity.this, strInfo, EtongToast.LENGTH_SHORT).show();
        }
    }
