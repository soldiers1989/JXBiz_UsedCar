...
public class UC_AdvancedFilter_RangeSeekBar extends View {

    private static final String DEBUG_TAG = "UC_AdvancedFilter_RangeSeekBar.java";
    ...
    public UC_AdvancedFilter_RangeSeekBar(Context context) {
        this(context, null, 0);
    }

    public UC_AdvancedFilter_RangeSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public UC_AdvancedFilter_RangeSeekBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void applyConfig(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.UC_AdvancedFilter_RangeSeekBar);

        mDuration = a.getInteger(R.styleable.UC_AdvancedFilter_RangeSeekBar_autoMoveDuration,
                DEFAULT_DURATION);

        mLeftCursorBG = a
                .getDrawable(R.styleable.UC_AdvancedFilter_RangeSeekBar_leftCursorBackground);
        mRightCursorBG = a
                .getDrawable(R.styleable.UC_AdvancedFilter_RangeSeekBar_rightCursorBackground);

        mTextColorNormal = a.getColor(R.styleable.UC_AdvancedFilter_RangeSeekBar_textColorNormal,
                Color.BLACK);
        mTextColorSelected = a.getColor(
                R.styleable.UC_AdvancedFilter_RangeSeekBar_textColorSelected,
                Color.rgb(242, 79, 115));

        mSeekbarColorNormal = a.getColor(
                R.styleable.UC_AdvancedFilter_RangeSeekBar_seekbarColorNormal,
                Color.rgb(218, 215, 215));
        mSeekbarColorSelected = a.getColor(
                R.styleable.UC_AdvancedFilter_RangeSeekBar_seekbarColorSelected,
                Color.rgb(242, 79, 115));

        mSeekbarHeight = (int) a.getDimension(
                R.styleable.UC_AdvancedFilter_RangeSeekBar_seekbarHeight, 10);
        mTextSize = (int) a.getDimension(R.styleable.UC_AdvancedFilter_RangeSeekBar_textSize, 15);
        mMarginBetween = (int) a.getDimension(
                R.styleable.UC_AdvancedFilter_RangeSeekBar_spaceBetween, 15);

        mTextArray = a.getTextArray(R.styleable.UC_AdvancedFilter_RangeSeekBar_markTextArray);
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (i == length - 1) {
                textDrawLeft = mSeekbarRect.right
                        + (mRightCursorBG.getIntrinsicWidth() / 2) - textWidth - 30;
            } else {
                textDrawLeft = mSeekbarRect.left + i * mPartLength - textWidth
                        / 2;
            }
            if (i % intervalValue == 0) {
                canvas.drawText(text2draw, textDrawLeft, mSeekbarRect.bottom
                        + mTextSize + 28, mPaint);
            }
...
//            }
    ...
    /**
     * @desc 设置数值的间隔
     * @createtime 2016/10/21 0021-19:35
     * @author wukefan
     */
