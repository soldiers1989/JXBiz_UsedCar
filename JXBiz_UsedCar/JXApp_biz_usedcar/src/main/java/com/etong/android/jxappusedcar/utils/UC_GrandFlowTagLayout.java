...
import com.etong.android.jxappusedcar.Interface.UC_OnInitSelectedPosition;
import com.etong.android.jxappusedcar.Interface.UC_OnTagClickListener;
import com.etong.android.jxappusedcar.Interface.UC_OnTagSelectListener;
 ...
 * 方法
 * Created by Ellison on 16/8/9.
 */
public class UC_GrandFlowTagLayout extends ViewGroup {

    private static final String TAG = UC_GrandFlowTagLayout.class.getSimpleName();
     ...
     * 点击事件的回调函数
     */
    UC_OnTagClickListener mOnTagClickListener;

    /**
     * 多选操作的回调监听
     */
    UC_OnTagSelectListener mOnTagSelectListener;
    ...
    public UC_GrandFlowTagLayout(Context context) {
        super(context);
    }

    public UC_GrandFlowTagLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public UC_GrandFlowTagLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0, childCount = getChildCount(); i < childCount; i++) {
            ...
            int childExpandSpec = MeasureSpec.makeMeasureSpec(Integer.MAX_VALUE >> 2, MeasureSpec.AT_MOST);
            //测量每一个子view的宽和高
            measureChild(childView, childExpandSpec, childExpandSpec);
    ...
    private void reloadData() {
        ...
        for (int i = 0; i < mAdapter.getCount(); i++) {
            ...
            if (mAdapter instanceof UC_OnInitSelectedPosition) {
                boolean isSelected = ((UC_OnInitSelectedPosition) mAdapter).isSelectedPosition(i);
                //判断一下模式
                ...
                public void onClick(View v) {
                    if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
                        if (mOnTagClickListener != null) {
                            mOnTagClickListener.onItemClick(UC_GrandFlowTagLayout.this, childView, j);
                        }
                        ...
                        if (mCheckedTagArray.get(j)) {
                            ...
                            if (mOnTagSelectListener != null) {
                                mOnTagSelectListener.onItemSelect(UC_GrandFlowTagLayout.this, new ArrayList<Integer>());
                            }
                            return;
                        }
                        ...
                        if (mOnTagSelectListener != null) {
                            mOnTagSelectListener.onItemSelect(UC_GrandFlowTagLayout.this, Arrays.asList(j));
                        }
                    } else if (mTagCheckMode == FLOW_TAG_CHECKED_MULTI) {
                        if (mCheckedTagArray.get(j)) {
                            mCheckedTagArray.put(j, false);
                        ...
                        if (mOnTagSelectListener != null) {
                            ...
                            mOnTagSelectListener.onItemSelect(UC_GrandFlowTagLayout.this, list);
                        }
                    }
                }
            });
    ...
    public void setOnTagClickListener(UC_OnTagClickListener onTagClickListener) {
        this.mOnTagClickListener = onTagClickListener;
    }

    public void setOnTagSelectListener(UC_OnTagSelectListener onTagSelectListener) {
