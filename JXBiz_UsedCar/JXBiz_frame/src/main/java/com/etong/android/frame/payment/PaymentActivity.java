...
public abstract class PaymentActivity extends BaseSubscriberActivity {
	...
	protected void onInit(Bundle savedInstanceState) {
		...
		api = WXAPIFactory.createWXAPI(this, MarkUtils.WECHAT_APP_ID);
		// 初始化
		onInit();
	}
	...
	protected void setPayInfo(PayInfo info) {
		...
		if ((System.currentTimeMillis() - mPayTime) > 2000) {
			...
			payInfo = info;
//			mPayProvider.payMent(payInfo);				// 使用FastJson来传递参数的情况
			mPayProvider.payMentSpliceUrl(payInfo);		// 使用拼接的形式来传递参数的情况
			loadStart("支付中,请稍后...", 0);
		}
	}
	...
	private void onAliPayFinish(HttpMethod method) {
		...
		if (tn != null && !TextUtils.isEmpty(tn)) {
			...
			toastMsg(method.data().getString("status"), method.data()
					.getString("errMsg"));
	}
	...
	private void onWeChatPayFinish(HttpMethod method) {
		...
		if (json != null) {
			...
			if (send) {
				L.d("WeChat Pay send success!");
			} else {
				L.d("WeChat Pay send fail!");
				toastMsg("支付失败");
			}
		} else {
			toastMsg(method.data().getString("status"), method.data()
	...
	private void onAlipayResult(String result) {
		...
		case 9000:
			loadStart("支付结果确认中，请稍后...", 0);
			timeCounterFinish = false;
			timeCounter = new TimeCounter(5 * 1000, 1 * 1000);
			timeCounter.start();
			mPayProvider.confirmPayment(payResult);		// 确认支付页面
			// 调用支付成功后的处理方法
...
//			paySuccess(payInfo, 9000, "支付成功");
	...
	public void onResp(BaseResp resp) {
		L.d("WeChat onPayFinish, errCode = " + resp.errCode);
	...
	@Subscriber(tag = CommonEvent.CONFIRM_ALIPAY)
	public void orderPayConfirm(HttpMethod httpMethod) {
		if (httpMethod == null) {
//			System.out.println("支付宝支付:支付成功");
//			getEventBus().post(payInfo, CommonEvent.PAY_SUCCESS);
			payFail(payInfo, 9000, "支付失败");
		}
