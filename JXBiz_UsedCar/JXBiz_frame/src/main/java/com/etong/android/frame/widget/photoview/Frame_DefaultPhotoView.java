 ...
 */
public class Frame_DefaultPhotoView extends ImageView {
    ...
    private Frame_RotateGestureDetector mRotateDetector;
    ...
    private RectF mClip;
    private Frame_DefaultInfo mFromInfo;
    private long mInfoTime;
    private Runnable mCompleteCallBack;

    private OnLongClickListener mLongClick;

    public Frame_DefaultPhotoView(Context context) {
        super(context);
        init();
    }

    public Frame_DefaultPhotoView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public Frame_DefaultPhotoView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        mRotateDetector = new Frame_RotateGestureDetector(mRotateListener);
    ...
    private Frame_OnRotateListener mRotateListener = new Frame_OnRotateListener() {
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private Runnable mClickRunnable = new Runnable() {
        ...
        public void run() {
            if (mClickListener != null) {
                mClickListener.onClick(Frame_DefaultPhotoView.this);
            }
        }
    };
    ...
    private GestureDetector.OnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            if (mLongClick != null) {
                mLongClick.onLongClick(Frame_DefaultPhotoView.this);
            }
        }
    ...
    public Frame_DefaultInfo getInfo() {
        RectF rect = new RectF();
        int[] p = new int[2];
        getLocation(this, p);
        rect.set(p[0] + mImgRect.left, p[1] + mImgRect.top, p[0] + mImgRect.right, p[1] + mImgRect.bottom);
        return new Frame_DefaultInfo(rect, mImgRect, mWidgetRect, mBaseRect, mScreenCenter, mScale, mDegrees, mScaleType);
    }

    public static Frame_DefaultInfo getImageViewInfo(ImageView imgView) {
        int[] p = new int[2];
        ...
        return new Frame_DefaultInfo(rect, imgRect, widgetRect, baseRect, screenCenter, 1, 0, imgView.getScaleType());
    }
     ...
     */
    public void animaFrom(Frame_DefaultInfo info) {
        if (isInit) {
            reset();

            Frame_DefaultInfo mine = getInfo();
    ...
    public void animaTo(Frame_DefaultInfo info, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
