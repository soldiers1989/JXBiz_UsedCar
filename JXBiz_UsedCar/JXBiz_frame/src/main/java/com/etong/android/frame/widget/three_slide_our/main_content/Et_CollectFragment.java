...
import com.etong.android.frame.widget.three_slide_our.utils.Find_Car_VechileCollect_Method;
 ...
 */
public class Et_CollectFragment extends BaseSubscriberFragment {
    ...
    // 设置选择某一个车系的监听事件
    public void setOnChooseCarSeriesListener(Et_OnChooseCarTypeListener listener) {
    ...
    private void initViews(View view) {
        ...
        mListAdapters = new ListAdapter<Et_VechileType>(
                getActivity(), R.layout.models_contrast_collect_list_item) {

            @Override
            protected void onPaint(View view, final Et_VechileType data, int position) {

                final TextView collect_title = (TextView) view.findViewById(R.id.models_contrast_txt_collect_title);
                collect_title.setText(data.getName());
                        ...
                        if (Et_ChooseCarType.IsNeedChecked) {//车型对比界面和参数对比选车界面
                            ...
                            if (map != null) {
                                ...
                                while (it.hasNext()) {
                                    ...
                                    if (String.valueOf(data.getName()).equals(id)) {
//                                        Toast.makeText(getActivity(), "该车型已添加", Toast.LENGTH_SHORT).show();
                                ...
                                if (!isHave) {
                                    Et_AddVechileStyle_Method.cartAdd(data);
        ...
        //得到缓存的收藏车辆
        List<Et_VechileType> mList = new ArrayList<Et_VechileType>();
        Map map = Find_Car_VechileCollect_Method.getVechileCollectInfo();
        Et_VechileType info = null;
        if (map != null) {
            Iterator it = map.entrySet().iterator();
            while (it.hasNext()) {
                Map.Entry entry = (Map.Entry) it.next();
                ...
                L.d("从本地获取到的数据:", data);
                info = JSON.parseObject(data, Et_VechileType.class);
                if (null != info) {
                    mList.add(info);
                }
            }
