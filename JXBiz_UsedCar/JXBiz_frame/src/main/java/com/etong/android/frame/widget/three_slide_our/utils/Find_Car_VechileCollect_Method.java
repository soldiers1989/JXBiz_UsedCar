package com.etong.android.frame.widget.three_slide_our.utils;
...
public class Find_Car_VechileCollect_Method {
     ...
     * @params
     */
    public static void cartAdd(Et_VechileType vechileCollect) {
        if (null != getVechileCollectInfo()) {
            Map oldMap = getVechileCollectInfo();
            oldMap.put(vechileCollect.getId(), vechileCollect);
            setVechileGoodsCartInfo(oldMap);
        } else {
            Map newMap = new HashMap<>();
            newMap.put(vechileCollect.getId(), vechileCollect);
