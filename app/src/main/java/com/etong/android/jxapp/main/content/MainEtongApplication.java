package com.etong.android.jxapp.main.content;
...
import com.etong.android.jxapp.main.user.MainUserInfo;
 ...
 */
public class MainEtongApplication extends BaseApplication {
    protected static final String TAG = "MessageEtongApplication";
    protected static MainEtongApplication application;
    protected MainUserInfo mUserInfo;
    protected ConfigInfo mConfigInfo;
    protected ImageProvider mImageProvider;
    static public MainEtongApplication getApplication() {
        return application;
    }
    @Override
...
//		mUserProvider.initialize(mHttpPublisher);
     ...
     * @return
     */
    public MainUserInfo getUserInfo() {
		if (null == mUserInfo) {
			// 如果没有用户信息则从缓存中读取
			String userShared = mSharedPublisher.getString(USER_SHARED);
			if (null != userShared && !userShared.isEmpty())
				mUserInfo = JSON.parseObject(userShared, MainUserInfo.class);
		}
		return mUserInfo;
	}

	public void setUserInfo(MainUserInfo userInfo) {
